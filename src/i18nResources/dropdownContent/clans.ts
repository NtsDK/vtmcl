...
export function* generateClanEntities(
  ...
  for (let value of gen) {
    ...
    const enArr = enLine.split(langSeparators)
      .map(R.trim).filter(el => el !== '')
    ;
    const ruArr = ruLine.split(langSeparators)
      .map(R.trim).filter(el => el !== '')
    ;
...
const clanGroupsArr = [...generateIdEnRuEntities(generateSequence(3, clanGroupsSource))];
