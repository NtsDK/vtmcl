...
const mentalFlawsArr = [...generateEnRuEntities(generateSequence(2, mentalFlawsSource))];
const mentalMeritsArr = [...generateEnRuEntities(generateSequence(2, mentalMeritsSource))];
const physicalFlawsArr = [...generateEnRuEntities(generateSequence(2, physicalFlawsSource))];
const physicalMeritsArr = [...generateEnRuEntities(generateSequence(2, physicalMeritsSource))];
const socialFlawsArr = [...generateEnRuEntities(generateSequence(2, socialFlawsSource))];
const socialMeritsArr = [...generateEnRuEntities(generateSequence(2, socialMeritsSource))];
const supernaturalFlawsArr = [...generateEnRuEntities(generateSequence(2, supernaturalFlawsSource))];
const supernaturalMeritsArr = [...generateEnRuEntities(generateSequence(2, supernaturalMeritsSource))];
...
export const c20_translateMeritsAndFlaws = makeTranslateFunction(allMeritsAndFlaws);


const meritsAndFlawsGroupsArr = [...generateIdEnRuEntities(generateSequence(3, meritsAndFlawsGroupsSource))];

const groupNames = R.indexBy(R.prop('id'), meritsAndFlawsGroupsArr) as Record<MeritsAndFlawsGroups, IdEnRuEntity>;
